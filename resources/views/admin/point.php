<?php
$body = [
    'title' => 'Cài đặt thông tin kiểu point',
    'desc'   => '',
    'keyword' => ''
];
$body['header'] = '
...
if (isset($_POST['AddPoint'])) {
    ...
    if ($isInsert) {
        $DB->insert("logs", [
            ...
            'createdate'    => time(),
            'action'        => "Thêm kiểu point (".$_POST['value'].") vào hệ thống."
        ]);
...
<div class="content-wrapper">
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-12">
                <div class="col-sm-6">
                    <h1 class="m-0">Danh sách kiểu point</h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="<?=BASE_URL('admin/');?>">Dashboard</a></li>
                        <li class="breadcrumb-item active">Danh sách kiểu point</li>
                    </ol>
                </div>
            </div>
        </div>
                    ...
                    <div class="card card-primary card-outline">
                        ...
                        <div class="card-body p-0">
                            <table class="table table-striped">
                                ...
                                <tbody>
                                    <?php foreach ($DB->get_list("SELECT * FROM `technique_info` WHERE `type` = 'point' ") as $rows) {?>
                                    <tr>
                                        <td><?=$rows['id'];?></td>
                                        <td><?=$rows['value'];?></td>
                        ...
                        <form action="" method="POST" enctype="multipart/form-data">
                            <div class="card-body">
                                <div class="form-group">
                                    <label for="exampleInputEmail1">Tên kiểu point</label>
                                    <input type="text" class="form-control" name="value"
                                        placeholder="Nhập tên kiểu point" required>
                                </div>
                            </div>
                            <div class="card-footer clearfix">
                                <button name="AddPoint" class="btn btn-info btn-icon-left m-b-10" type="submit"><i
...
function RemoveRow(id) {
    cuteAlert({
        ...
        message: "Bạn có chắc chắn muốn xóa kiểu point ID " + id + " không ?",
        confirmText: "Đồng Ý",
        cancelText: "Hủy"
    }).then((e) => {
        if (e) {
            $.ajax({
                url: "<?=BASE_URL("ajaxs/admin/remove-point.php");?>",
                method: "POST",
                dataType: "JSON",
                data: {
                    id: id
