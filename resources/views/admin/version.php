<?php
$body = [
    'title' => 'Cài đặt thông tin phiên bản',
    'desc'   => '',
    'keyword' => ''
];
$body['header'] = '
...
if (isset($_POST['AddVersion'])) {
    ...
    if ($isInsert) {
        $DB->insert("logs", [
            ...
            'createdate'    => time(),
            'action'        => "Thêm phiên bản (".$_POST['value'].") vào hệ thống."
        ]);
                    ...
                    <div class="card card-primary card-outline">
                        ...
                        <div class="card-body p-0">
                            <table class="table table-striped">
                                ...
                                <tbody>
                                    <?php foreach ($DB->get_list("SELECT * FROM `technique_info` WHERE `type` = 'version' ") as $rows) {?>
                                    <tr>
                                        <td><?=$rows['id'];?></td>
                                        <td><?=$rows['value'];?></td>
                        ...
                        <form action="" method="POST" enctype="multipart/form-data">
                            <div class="card-body">
                                <div class="form-group">
                                    <label for="exampleInputEmail1">Tên phiên bản</label>
                                    <input type="text" class="form-control" name="value"
                                        placeholder="Nhập tên phiên bản" required>
                                </div>
                            </div>
                            <div class="card-footer clearfix">
                                <button name="AddVersion" class="btn btn-info btn-icon-left m-b-10" type="submit"><i
...
function RemoveRow(id) {
    cuteAlert({
        ...
        message: "Bạn có chắc chắn muốn xóa phiên bản ID " + id + " không ?",
        confirmText: "Đồng Ý",
        cancelText: "Hủy"
    }).then((e) => {
        if (e) {
            $.ajax({
                url: "<?=BASE_URL("ajaxs/admin/remove-version.php");?>",
                method: "POST",
                dataType: "JSON",
                data: {
                    id: id
