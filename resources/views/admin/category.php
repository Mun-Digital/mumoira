<?php
$body = [
    'title' => 'Cài đặt thông tin thể loại',
    'desc'   => '',
    'keyword' => ''
];
$body['header'] = '
...
if (isset($_POST['AddCategory'])) {
    ...
    if ($isInsert) {
        $DB->insert("logs", [
            ...
            'createdate'    => time(),
            'action'        => "Thêm thể loại (".$_POST['value'].") vào hệ thống."
        ]);
                    ...
                    <div class="card card-primary card-outline">
                        ...
                        <div class="card-body p-0">
                            <table class="table table-striped">
                                ...
                                <tbody>
                                    <?php foreach ($DB->get_list("SELECT * FROM `technique_info` WHERE `type` = 'category' ") as $rows) {?>
                                    <tr>
                                        <td><?=$rows['id'];?></td>
                                        <td><?=$rows['value'];?></td>
...
function RemoveRow(id) {
    cuteAlert({
        ...
        message: "Bạn có chắc chắn muốn xóa thể loại ID " + id + " không ?",
        confirmText: "Đồng Ý",
        cancelText: "Hủy"
    }).then((e) => {
        if (e) {
            $.ajax({
                url: "<?=BASE_URL("ajaxs/admin/remove-category.php");?>",
                method: "POST",
                dataType: "JSON",
                data: {
                    id: id
